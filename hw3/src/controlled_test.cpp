...
using namespace std;
  ...
  // for 3 T turtles
  // starting positions: A = (8,10), B = (11,6), C = (4,5).
  const int MAX_TTURTLES = 3; //this number can be changed 
  int T_turt_pos_x[MAX_TTURTLES] = {8, 11, 4};
  int T_turt_pos_y[MAX_TTURTLES] = {10, 6, 5};

  // for X turtles
  const int MAX_XTURTLES = 1; //this number can be changed 
  int X_turt_pos_x[MAX_XTURTLES] = {0};
  int X_turt_pos_y[MAX_XTURTLES] = {11};
...
void createTurtles(char tType, int cnt) {
  ...
  for (i=0; i<cnt; i++) {
     ...
     if (tType == 'X') {
        req.x = X_turt_pos_x[i];
        req.y = X_turt_pos_y[i];
        ...
        req.x = T_turt_pos_x[i];
        req.y = T_turt_pos_y[i];
